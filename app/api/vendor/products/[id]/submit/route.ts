...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: product } = await ctx.supabase.from("vendor_products").select("*, product_images(id), product_variants(*)").eq("id", id).eq("vendor_id", ctx.vendor.id).single();
  ...
  if (product.has_variants) {
    ...
    if (!variants.length) return NextResponse.json({ error: "Please add at least one variant." }, { status: 400 });
    if (variants.some((variant: any) => variant.price === null || variant.price === undefined || Number(variant.price) <= 0)) return NextResponse.json({ error: "Please complete price for all variants before submitting." }, { status: 400 });
    if (variants.some((variant: any) => variant.stock === null || variant.stock === undefined || Number(variant.stock) < 0)) return NextResponse.json({ error: "Please complete stock for all variants before submitting." }, { status: 400 });
  } else {
