...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data, error } = await ctx.supabase.from("vendor_products").update({ status: "rejected", rejected_at: new Date().toISOString(), rejection_reason: parsed.data.rejection_reason ?? null }).eq("id", id).eq("status", "submitted").select().single();
