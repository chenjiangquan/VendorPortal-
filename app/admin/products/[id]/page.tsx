  ...
  return (
    <DashboardShell role="admin" title={product?.title ?? "Product"}>
      <div className="grid gap-5 lg:grid-cols-[1fr_320px]">
            ...
            <table className="w-full text-left text-sm">
              ...
              <tbody className="divide-y divide-line">
                {(product?.product_variants ?? []).map((variant: any) => (
                  <tr key={variant.id}><td className="px-3 py-3">{[variant.option1_value, variant.option2_value, variant.option3_value].filter(Boolean).join(" / ") || "Default Title"}</td><td className="px-3 py-3">{variant.sku}</td><td className="px-3 py-3">{formatCurrency(variant.price ?? product?.price)}</td><td className="px-3 py-3">{variant.stock}</td></tr>
                ))}
              </tbody>
            </table>
          </div>
        ...
        {product && <AdminProductReviewPanel product={{ ...product, shopifyAdminUrl: adminUrl }} />}
      </div>
    </DashboardShell>
  );
}
