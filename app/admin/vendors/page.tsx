...
function Table({ rows }: { rows: any[] }) {
  return <div className="overflow-hidden rounded-2xl border border-line bg-white shadow-sm"><table className="w-full text-left text-sm"><thead className="bg-panel text-xs uppercase text-slate-500"><tr><th className="px-4 py-3">Company</th><th className="px-4 py-3">Email</th><th className="px-4 py-3">Commission</th><th className="px-4 py-3">Status</th></tr></thead><tbody className="divide-y divide-line">{rows.map((vendor) => <tr key={vendor.id} className="hover:bg-panel/70"><td className="px-4 py-3"><Link className="font-medium hover:underline" href={`/admin/vendors/${vendor.id}`}>{vendor.company_name}</Link></td><td className="px-4 py-3">{vendor.email}</td><td className="px-4 py-3">{vendor.commission_rate}%</td><td className="px-4 py-3"><StatusBadge status={vendor.status} /></td></tr>)}</tbody></table></div>;
}
